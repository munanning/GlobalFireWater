...
# === 第一步：绘制特征重要性图 === #
# 加载csv数据
importance_data <- read.csv("sorted_feature_importance_lake.csv")
...
# 归一化标准差
importance_data$std_normalized <- importance_data$std / max(importance_data$std) * 2  # 调整这个乘数以控制标准差线的最大长度

# 指数平滑
importance_data$X.IncMSE_smoothed <- log(importance_data$X.IncMSE + 1)
importance_data$X.IncMSE <- (importance_data$X.IncMSE^0.7) * 3 # 先开根号再乘以4
...
vi_plot <- ggplot(importance_data, aes(x = X.IncMSE, y = reorder(Feature, X.IncMSE, FUN = mean))) +
  ...
  theme_classic() +
  labs(x = "Importance (%IncMSE)", y = "", fill = "Category", title = "Driver Importance For Lakes") +
  theme(
...
pdp_plot <- pdp_rf$agr_profiles %>%
  ...
  mutate(
    ...
    `_x_` = case_when(
      `_vname_` == "wind" & `_x_` < 0.5 ~ NA_real_,
      `_vname_` == "slope" & `_x_` < 1.0 ~ NA_real_,
      `_vname_` == "slope_basin" & `_x_` < 1.0 ~ NA_real_,
      TRUE ~ `_x_`
    ),

    `_x_` = ifelse(`_vname_` %in% c("MIN", "MEAN"), -`_x_`, `_x_`),  # 对x值取相反数
    
    `_yhat_` = ifelse(`_vname_` %in% c("wind", "slope", "slope_basin") & is.na(`_x_`), NA, `_yhat_`),  # 对应的 yhat 也设为 NA
    
    # 对 wind 和 slope 特征的 x 重新归一化，其他特征保持原样
    `_x_` = ifelse(`_vname_` %in% c("wind", "slope", "slope_basin"),
    ...
    `_yhat_` = ifelse(`_vname_` %in% c("wind", "slope", "slope_basin"),
  ...
  drop_na(`_x_`, `_yhat_`) %>%  # 删除slope中被设置为NA的x和yhat值
  ungroup() %>%
  ggplot(aes(x = `_x_`, y = `_yhat_`)) +
  geom_line(color = "black", size = .5) +
  facet_wrap(~ `_vname_`, scales = "fixed", ncol = 1) +  # 保持每个特征的 x 轴独立
  theme_void() +  # 清除背景和网格线
  theme(
...
# === 第七步：将PDP叠加到特征重要性图上 === #
vi_pdp_plot <- vi_plot + 
  inset_element(pdp_plot, left = 0.0, bottom = 0.01, right = .43, top = 0.99)

# 打印合成图
print(vi_pdp_plot)

# 保存合成图
ggsave("vi_pdp_plot_combined_lake_std_100.png", vi_pdp_plot, width = 8, height = 10, dpi = 450, bg = "transparent")
