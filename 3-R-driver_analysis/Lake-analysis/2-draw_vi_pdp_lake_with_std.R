...
# Custom transformation function for scaling
...
# Load feature name and category mappings
source("mappings.R")
...
# Create a feature importance plot with ggplot2, adding normalized standard deviation lines
vi_plot <- ggplot(importance_data, aes(x = X.IncMSE, y = reorder(Feature, X.IncMSE, FUN = mean))) +
  ...
  geom_segment(aes(x = X.IncMSE - std_normalized, xend = X.IncMSE + std_normalized, y = reorder(Feature, X.IncMSE), yend = reorder(Feature, X.IncMSE)),
               color = "black", size = 0.8, alpha = 0.7) +  # Add normalized standard deviation
...
# Select specified features and target variable
...
df_filtered <- df %>%
  drop_na()  # Remove rows containing NA values

# === Step 4: Create an explainer object using DALEXtra's explain_tidymodels === #
explainer_rf <- explain_tidymodels(
  rf_model,  # Use the loaded model object
  data = dplyr::select(df_filtered, -change4),  # Filtered dataset without the target variable
...
pdp_plot <- pdp_rf$agr_profiles %>%
  ...
  facet_wrap(~ `_vname_`, scales = "fixed", ncol = 1) +  # Keep x-axis independent for each feature
