...
for (i in 1:num_runs) {
  ...
  if (current_oob_mse < best_oob_mse) {
    ...
    save(rf_model, file = "best_rf_model_lake.RData")
  }
...
write.csv(result_df, 'sorted_feature_importance_lake.csv', row.names=FALSE)

# Output relevant information
cat("Median and standard deviation of %IncMSE have been saved to 'sorted_feature_importance_lake.csv'\n")
cat(paste("The model with the lowest OOB MSE (Run", best_run, ") has been saved as 'best_rf_model_lake.RData'\n"))
