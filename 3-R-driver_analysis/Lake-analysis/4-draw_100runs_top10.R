...
# 设置文件夹路径
folder_path <- "100runs_lake"

# 设置感兴趣的特征
target_features <- c('Rainfall', 'slope', 'MEAN', 'area')
...
vi_plot <- ggplot(feature_counts, aes(x = Proportion, y = reorder(Feature, Proportion), fill = Category)) +
  ...
  labs(x = "Proportion", y = NULL, title = "Frequency in Top 10 Importance for Lakes") +
...
ggsave("feature_importance_frequency_lake.png", plot = vi_plot, width = 7, height = 4, dpi = 300)
